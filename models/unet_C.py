...
class UNet(nn.Module):
    def __init__(self, in_channels=3, num_classes=1, bilinear=True, base_c=32):
        ...
        self.coarse_head = SegHead(base_c, num_classes)
        self.boundary_head = SegHead(base_c, 1)
        self.uncertainty_head = SegHead(base_c, 1)

        refine_in_channels = base_c + num_classes + 1 + 1
        self.refinement_head = RefinementHead(refine_in_channels, num_classes, hidden_channels=base_c)
    ...
    def forward(self, x):
        ...
        x = self.up2(x, x3, p3)
        x = self.up3(x, x2, p2)
        f_dec = self.up4(x, x1, p1)

        coarse_logits = self.coarse_head(f_dec)
        boundary_logits = self.boundary_head(f_dec)
        uncertainty_logits = self.uncertainty_head(f_dec)

        coarse_prob = torch.sigmoid(coarse_logits)
        boundary_prob = torch.sigmoid(boundary_logits)
        uncertainty_map = torch.sigmoid(uncertainty_logits)

        refine_input = torch.cat([f_dec, coarse_prob, boundary_prob, uncertainty_map], dim=1)
        delta_logits = self.refinement_head(refine_input)

        final_logits = coarse_logits + uncertainty_map * delta_logits
